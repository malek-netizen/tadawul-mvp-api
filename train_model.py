...
# --- دوال فنية (كما في app.py)
def sma(series, window): return series.rolling(window).mean()
def ema(series, span): return series.ewm(span=span, adjust=False).mean()
def rsi(close, period=14):
    delta = close.diff(); gain = delta.where(delta>0,0.0); loss=(-delta).where(delta<0,0.0)
    return 100-(100/(1+(gain.rolling(period).mean()/(loss.rolling(period).mean()+1e-9))))
def macd(close, fast=12, slow=26, signal=9):
    ema_fast = ema(close, fast); ema_slow=ema(close, slow)
    macd_line=ema_fast-ema_slow; sig_line=ema(macd_line, signal); hist=macd_line-sig_line
    return macd_line,sig_line,hist
def bollinger(close, window=20, num_std=2):
    mid = sma(close, window); std=close.rolling(window).std()
...
def fetch_yahoo_prices(ticker: str, range_="2y", interval="1d"):
    ...
    for _ in range(3):
        for base in bases:
            try:
                url=f"{base}{ticker}"; r=requests.get(url, params={"range":range_,"interval":interval}, headers=headers, timeout=25)
                if r.status_code!=200: continue
                js=(r.json().get("chart",{}).get("result") or [None])[0]
                if not js: continue
                quote=(js.get("indicators",{}).get("quote") or [None])[0]
                if not quote: continue
                df=pd.DataFrame({"open":quote.get("open",[]),"high":quote.get("high",[]),"low":quote.get("low",[]),
                                 "close":quote.get("close",[]),"volume":quote.get("volume",[])}).dropna(subset=["close"])
...
def main():
    ...
    y=data["label"].astype(int).values; X=data[FEATURE_COLS].astype(float).values
