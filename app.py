...
# =========================
# Tadawul MVP API (Rules v1.4 - Short-term Scalping)
# هدف: +5% | وقف: ديناميكي (2*ATR) مع حد أقصى -2% | تاسي فقط
# مؤشرات: ADX, Stochastic, CMF, حجم, نسبة مخاطرة/مكافأة
# الإصدار: 2.4.0 – معدل للدخول القصير المدى
# =========================

app = FastAPI(title="Tadawul MVP API", version="2.4.0-short-term")
...
# قواعد الدخول بعد التعديل (قيم مخففة للتداول القصير)
RSI_MIN = 40                 # 40-65 يسمح بمرونة أكثر
RSI_MAX = 65
RSI_OVERBOUGHT = 70

ADX_MIN = 20                 # اتجاه متوسط على الأقل
STOCH_MAX = 85               # يُسمح بقرب التشبع ولكن ليس تجاوز 85
CMF_MIN = -0.05              # تدفق نقدي شبه محايد (قليل من السحب مقبول)
VOL_RATIO_MIN = 1.0          # حجم مساوٍ للمتوسط على الأقل
RR_MIN = 2.0                 # نسبة مخاطرة/مكافأة ≥ 2 (خسارة ≤2.5% مقابل ربح 5%)
...
ML_THRESHOLD = 0.60          # عتبة النموذج (يُستخدم في الدرجة المركبة)
TOP10_WORKERS = 6
...
def passes_rules(df_ohlc: pd.DataFrame, feat_df: pd.DataFrame, stop_loss: float = None):
    ...
    if not (RSI_MIN <= rsi14 <= RSI_MAX):
        reasons.append(f"مرفوض: RSI خارج النطاق الآمن ({RSI_MIN}-{RSI_MAX}).")
    ...
    # الشروط الإضافية بالقيم المخففة
...
def top10(max_workers: int = TOP10_WORKERS):
    ...
    payload = {
        ...
        "errors": errors,
        "source": TICKERS_PATH,
        "note": "Rules v1.4-short-term. Top10 cached for 10 minutes.",
        "cached": False,
